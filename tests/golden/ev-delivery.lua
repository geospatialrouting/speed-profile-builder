...
--
-- profile:     ev-delivery (mode: van)
-- spec chain:  ev-delivery.yaml <- van-urban.yaml <- car.yaml
-- fingerprint: 9da24aa8161c64f3
-- summary:     Battery-electric 3.5 t delivery van, LEZ-exempt, range-limited.
--
...
local PROFILE_NAME = "ev-delivery"
local DEFAULT_SPEED = 22
local GLOBAL_FACTOR = 0.9
local MAX_LEGAL_SPEED = 100
...
local access_hierarchy = { "goods", "motor_vehicle", "vehicle", "access" }
local access_allow = {
  ["yes"] = true,
  ["designated"] = true,
  ["permissive"] = true,
...
local allow_through_destination = true
...
local zone_rules = {
  { id = "lez-exempt", tag = "low_emission_zone", value = nil, block = false, factor = 0.9967 },
  { id = "congestion-charge", tag = "congestion_charge", value = nil, block = false, factor = 0.5556 },
}

local FERRY_ALLOWED = false
local FERRY_SPEED = 12
local FERRY_FACTOR = 0.3333
local TOLL_AVOID = false
local TOLL_FACTOR = 0.8333
